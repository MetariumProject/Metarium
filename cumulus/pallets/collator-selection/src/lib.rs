...
pub mod pallet {
		...
		pallet_authorship::EventHandler<T::AccountId, BlockNumberFor<T>> for Pallet<T>
	{
		fn note_author(author: T::AccountId) {
			...
			// 	.div(2u32.into());
			// Set reward to 1/5 of the minimum balance of the currency.
			let mut reward = T::Currency::minimum_balance()
				.div(5u32.into());
			let mut remaining_pot_balance = T::Currency::free_balance(&pot)
				.checked_sub(&T::Currency::minimum_balance())
				.unwrap_or_else(Zero::zero);
			remaining_pot_balance = remaining_pot_balance.checked_sub(&reward).unwrap_or_else(Zero::zero);
			if remaining_pot_balance == Zero::zero() {
				// Pot is empty, no reward to give.
				reward = Zero::zero();
			}
			// `reward` is 0.0002 MAI or 0 MAI, this should never fail.
